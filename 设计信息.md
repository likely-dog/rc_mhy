...
本项目采用多 Agent 分工，将需求理解、方案细化、代码实现和独立审计拆开，降低单一 Agent 同时承担“出题、答题、验题”造成的盲区。本文说明 Agent 的角色、交接物、质量门禁和迭代闭环；系统本身的技术架构见 [docs/design.md](docs/design.md)，AI 使用取舍见 [docs/ai-usage.md](docs/ai-usage.md)。

协作遵循三项原则：

1. **文档驱动**：每个阶段通过版本化文件交接，关键结论可追溯。
2. **职责分离**：规划者定义问题，执行者实现，审计者独立找缺口。
3. **人工把关**：涉及互斥方案、需求冲突或范围变化时，不由 Agent 静默决定；先记录问题，确认后再执行。
...
- Planner 产出了初版 `prd_demo.md`；Re-Planner 将其细化为可执行的 `prd_final.md`。
- Auditor 识别的设计缺口被分级记录，Executor 在实现前进一步交叉核对并形成 12 项决策基线。
- Executor 完成 API、数据库、队列、worker、重试、DLQ、恢复逻辑以及配套测试和文档。
- 最终自动化测试共 45 项通过；覆盖率、Docker 冒烟和入口基准结果见 `docs/test-report.md`。
- 未采用的过度设计和 Executor 自主取舍均在 `docs/ai-usage.md` 中披露，便于后续复盘。

因此，多 Agent 的价值不只是“轮流生成内容”，而是通过明确的输入输出、独立审查和证据化闭环，让需求、实现与验收之间保持可追踪的一致性。
