...
| 语言     | Python **3.11+**                                             |
| 主要依赖 | fastapi, uvicorn[standard], httpx, redis, arq, sqlalchemy[asyncio], aiosqlite, pydantic, pydantic-settings, pyyaml, structlog, tenacity, pytest, pytest-asyncio, respx |
...
├── app/
│   ├── __init__.py
│   ├── main.py                  # FastAPI 入口 + lifespan（启动扫 pending）
...
│   ├── queue/
│   │   ├── __init__.py
│   │   └── redis_stream.py      # 封装 XADD / XREADGROUP / XACK / XCLAIM
│   ├── worker/
│   │   ├── __init__.py
│   │   ├── settings.py          # arq WorkerSettings
│   │   └── tasks.py             # dispatch_notification task
│   ├── dispatcher/
│   │   ├── __init__.py
...
| ------------------ | --------- | ---------------- | ----------------------------------- |
| `id`               | TEXT(26)  | PK               | ULID                                |
| `idempotency_key`  | TEXT(128) | UNIQUE, NOT NULL | 幂等键                              |
...
**渲染规则**：
- 模板引擎用 Jinja2（`jinja2` 属于 fastapi 传递依赖，无需单独加）；如不想引入，用 `string.Template` 简易方案，只支持 `${payload.xxx}` 单层访问。
- `${ENV}` 在加载时展开一次；`{{ payload.* }}` 在每次分发时渲染。
...
async def post_notify(req: NotifyRequest, idempotency_key: str, session):
    # 1) 幂等查询
    existing = await session.get_by_key(idempotency_key)
    if existing:
        return NotifyResponse(id=existing.id, status=existing.status, idempotent_hit=True), 200

    # 2) 校验 vendor 存在
    if req.vendor not in vendor_registry:
        raise HTTPException(400, f"unknown vendor: {req.vendor}")

    # 3) 落库（status=pending, attempts=0）
    nid = ulid()
    await session.insert_notification(nid, idempotency_key, req)

    # 4) 入队（XADD 到 Redis Streams）
...
> ⚠️ 3 和 4 之间理论上有极小的失败窗口。MVP 用「启动扫 pending」兜底：进程启动时把 `status=pending AND updated_at < now-30s` 的记录重新入队。

### 7.2 Worker 分发
```python
async def dispatch_notification(ctx, notification_id):
    n = await db.get(notification_id)
    if n is None or n.status in ("success", "dead"):
        return  # 幂等：已处理过就跳

    await db.mark_running(n.id, attempts=n.attempts + 1)
    try:
        vendor = vendor_registry[n.vendor]
        req = render_request(vendor, n.payload)
        resp = await http_client.request(**req, timeout=vendor.timeout_seconds)
        cls = classify(resp)
    ...
    if cls == SUCCESS:
        await db.mark_success(n.id)
    elif cls == NON_RETRYABLE:
        await db.mark_dead(n.id, reason="non_retryable_4xx", last=resp)
        await db.write_dead_letter(n)
    else:  # RETRYABLE
        if n.attempts >= MAX_ATTEMPTS:
            await db.mark_dead(n.id, reason="max_retries_exceeded", last=resp)
            await db.write_dead_letter(n)
        else:
            delay = respect_retry_after(resp) or next_delay(n.attempts)
            await db.mark_pending(n.id, next_retry_at=now_utc() + delay)
            await stream.enqueue_delayed(n.id, delay)
```
...
3. 完成幂等：同 key 命中返回 200 + `idempotent_hit=true`
4. 幂等 key 的 UNIQUE 索引 + 竞态处理（并发插同一 key → 捕获 `IntegrityError` → 查 & 返）
...
**任务**
1. 实现 `app/queue/redis_stream.py`：XADD / XREADGROUP / XACK；启动时 `XGROUP CREATE MKSTREAM`（幂等）
2. 实现 `app/dispatcher/renderer.py`（Jinja2 或 string.Template）
3. 实现 `app/dispatcher/http_client.py`（AsyncClient 单例 + 合理连接池）
4. 实现 `app/worker/tasks.py` 的 `dispatch_notification`（**只处理成功路径**，先不管重试）
5. 更新 `docker-compose.yml` 加 worker 服务
...
4. 实现启动扫 pending：`SELECT id FROM notifications WHERE status='pending' AND (next_retry_at IS NULL OR next_retry_at <= now())` 重新入队
...
| 有副作用的服务 | `dispatch_notification` (调 DB + Redis + HTTP)  | 集成测试：真 DB (SQLite in-memory) + 真 Redis (miniredis 或 docker) + mock HTTP (respx) |
...
- 时间相关测试**必须** freeze，否则 CI 抖动会 flaky。
...
- [ ] **时间可控**：涉及退避 / 超时的用例用 `freezegun` 或注入时钟
...
```markdown
## AI 帮上的
- 生成 arq WorkerSettings 骨架，节省 15 分钟查文档时间。
- 生成 respx mock 用例模板。
- 建议用 ULID 而不是 UUID v4，被采纳。

## 没采纳的建议
- AI 提议引入 Celery + Redis + beat 做延迟队列 → 拒绝：arq 已够，Celery 配置成本高 3 倍。
...
| 卡点                        | 应对                                                         |
| --------------------------- | ------------------------------------------------------------ |
| Redis Streams API 记不清    | 直接看官方 `XADD` / `XREADGROUP` / `XACK` / `XCLAIM` doc，别自己发明 |
| arq 用法不熟                | 参考 arq 官方 example，最小化 WorkerSettings                 |
